...
class MatukioModelBookingfields extends JModelAdmin {

    var $_bookingfields = null;
    var $_total = null;
    ...
    private function _buildQuery() {
        ...
        $orderby = $this->_buildContentOrderBy($context, 'cc.field_name');
    ...
    private function _buildContentOrderBy($context, $cc_or_a) {
        ...
        $appl = JFactory::getApplication();
        $filter_order = $appl->getUserStateFromRequest($context . 'filter_order', 'filter_order', 'cc.field_name', 'cmd'); // Category tree works with id not with ordering
        $filter_order_dir = $appl->getUserStateFromRequest($context . 'filter_order_Dir', 'filter_order_Dir', '', 'word');

        if ($filter_order == 'cc.field_name') {
            $orderby = ' ORDER BY cc.field_name ' . $filter_order_dir;
        } else {
    ...
    private function _buildContentWhere($context) {
        ...
        if ($search) {
            $where[] = 'LOWER(cc.field_name) LIKE ' . $this->_db->Quote('%' . $search . '%');
        }
        if ($filter_state) {
            if ($filter_state == 'P') {
     ...
     * @since	1.6
     */
    public function getTable($type = 'Bookingfields', $prefix = 'Table', $config = array())
    {
    ...
    public function getForm($data = array(), $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_matukio.bookingfields', 'bookingfields', array('control' => 'jform', 'load_data' => $loadData));
    ...
    protected function loadFormData()
    {
        ...
        $data = JFactory::getApplication()->getUserState('com_matukio.edit.bookingfields.data', array());
